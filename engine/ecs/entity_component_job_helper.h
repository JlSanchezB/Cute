...
namespace ecs
{
	...
	struct JobBucketDataWithCapture
	{
		...
		static void Job(void* bucket_job_data)
		{
			JobBucketDataWithCapture* this_bucket_job_data = reinterpret_cast<JobBucketDataWithCapture*>(bucket_job_data);
			...
			for (InstanceIndexType instance_index = this_bucket_job_data->begin_instance; instance_index < this_bucket_job_data->end_instance; ++instance_index)
			{
				...
				//Call kernel
				caller_helper<DATABASE_DECLARATION>(this_bucket_job_data->kernel, this_bucket_job_data->instance_iterator, instance_index, std::make_index_sequence<sizeof...(COMPONENTS)>(), this_bucket_job_data->components);
			}
		}
	};
