...
{
	DDS_ALPHA_MODE CalculateAlphaMode(const uint8_t* dds_file, size_t size)
	{
		...
		uint32_t dwMagicNumber = *(const uint32_t*)(dds_file);
		if (dwMagicNumber != DDS_MAGIC)
		{
			return DDS_ALPHA_MODE_UNKNOWN;
		}

		auto header = reinterpret_cast<const DDS_HEADER*>(dds_file + sizeof(uint32_t));

		return GetAlphaMode(header);
	}
		...
		std::vector<D3D12_SUBRESOURCE_DATA>& sub_resources, D3D12_SHADER_RESOURCE_VIEW_DESC& dx12_shader_resource_view_desc)
	{
		...
		const uint8_t* bitData = dds_file + offset;

		UINT bitSize = static_cast<UINT>(size - offset);
		...
		hr = FillInitData(width, height, depth, mipCount, arraySize,
			...
			size, bitSize, bitData,
			twidth, theight, tdepth, skipMip, sub_resources);

		if (SUCCEEDED(hr))
		{
			size_t reservedMips = mipCount;
			if (flags & DDS_LOADER_MIP_RESERVE)
			{
				reservedMips = std::min<size_t>(D3D12_REQ_MIP_LEVELS, CountMips(width, height));
			}

			if (flags & DDS_LOADER_FORCE_SRGB)
			{
				format = MakeSRGB(format);
			}

			d12_resource_desc.Width = static_cast<UINT>(width);
			d12_resource_desc.Height = static_cast<UINT>(height);
			d12_resource_desc.MipLevels = static_cast<UINT16>(mipCount);
			d12_resource_desc.DepthOrArraySize = (resDim == D3D12_RESOURCE_DIMENSION_TEXTURE3D) ? static_cast<UINT16>(depth) : static_cast<UINT16>(arraySize);
			d12_resource_desc.Format = format;
			...
			d12_resource_desc.Dimension = resDim;
		}
