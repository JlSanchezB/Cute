...
namespace display
{
	...
	void Context::SetRootSignature(const WeakRootSignatureHandle & root_signature_handle)
	{
		auto dx12_context = reinterpret_cast<DX12Context*>(this);
		Device* device = dx12_context->device;
		const auto& command_list = dx12_context->command_list;
		...
		dx12_context->current_root_signature = root_signature_handle;
	}
	...
	void Context::SetConstants(size_t root_parameter, const void * data, size_t num_constants)
	{
		auto dx12_context = reinterpret_cast<DX12Context*>(this);
		Device* device = dx12_context->device;
		const auto& command_list = dx12_context->command_list;

		auto& root_signature = device->Get(dx12_context->current_root_signature);
	...
	void Context::SetConstantBuffer(size_t root_parameter, const WeakConstantBufferHandle & constant_buffer_handle)
	{
		auto dx12_context = reinterpret_cast<DX12Context*>(this);
		Device* device = dx12_context->device;
		const auto& command_list = dx12_context->command_list;
	...
	void Context::SetUnorderedAccessBuffer(size_t root_parameter, const WeakUnorderedAccessBufferHandle & unordered_access_buffer_handle)
	{
		auto dx12_context = reinterpret_cast<DX12Context*>(this);
		Device* device = dx12_context->device;
		const auto& command_list = dx12_context->command_list;
	...
	void Context::SetShaderResource(size_t root_parameter, const WeakShaderResourceHandle & shader_resource_handle)
	{
		auto dx12_context = reinterpret_cast<DX12Context*>(this);
		Device* device = dx12_context->device;
		const auto& command_list = dx12_context->command_list;
	...
	void Context::SetDescriptorTable(size_t root_parameter, const WeakDescriptorTableHandle & descriptor_table_handle)
	{
		auto dx12_context = reinterpret_cast<DX12Context*>(this);
		Device* device = dx12_context->device;
		const auto& command_list = dx12_context->command_list;
	...
	void Context::SetDescriptorTable(size_t root_parameter, const WeakSamplerDescriptorTableHandle& sampler_descriptor_table_handle)
	{
		auto dx12_context = reinterpret_cast<DX12Context*>(this);
		Device* device = dx12_context->device;
		const auto& command_list = dx12_context->command_list;
